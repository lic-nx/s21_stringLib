...
START_TEST(test_s21_memchr_2) {
  ...
  ck_assert_ptr_eq(s21_memchr("Hello world\0", 'w', 4),
                   memchr("Hello world\0", 'w', 4));
...
START_TEST(s21_sprintf_test_i) {
  ...
  short h = 34;
  long l = 2353423523424;
  int r1 = sprintf(str1, "%+50.i%-10.20i%10.i%12i%7.5i%-10.8i%+10.1li%8.25hi",
...
START_TEST(s21_sprintf_test_c) {
  ...
  char str2[1000] = {0};
  wchar_t c1 = 'T', c2 = 'Z';
  int t1 = sprintf(str1, "%10c %-10c %25c %-30c %25c %8c %2c %8c %lc %-50lc",
                   'a', 'f', 'R', 'Q', 10, 'P', 'L', 20, c1, c2);
  int t2 =
      s21_sprintf(str2, "%10c %-10c %25c %-30c %25c %8c %2c %8c %lc %-50lc",
                  'a', 'f', 'R', 'Q', 10, 'P', 'L', 20, c1, c2);
  ck_assert_pstr_eq(str1, str2);
...
START_TEST(s21_sprintf_test_e) {
  char string1[100] = "";
  char string2[100] = "";
  int r1 = sprintf(string1, "[%10.4e], [%+-*.*E], [%15e]%-15e", 21.8924, 30, 6,
                   0.0007812, 0.0, 0.);
  int r2 = s21_sprintf(string2, "[%10.4e], [%+-*.*E], [%15e]%-15e", 21.8924, 30,
                       6, 0.0007812, 0.0, 0.);
...
START_TEST(s21_sprintf_test_E) {
  ...
  int r1 = sprintf(
      string1,
      "[%+-25.4E][%15.*E][%E] %10.3le %10.3lg %10.3lf %-5Le%+e%+Le% Le",
      3456.4325, 3, 194.74543, 0.0, 58787.878958, 58787.878958, 58787.878958,
      (long double)0.2, 0., (long double)0., (long double)0.);
  int r2 = s21_sprintf(
      string2,
      "[%+-25.4E][%15.*E][%E] %10.3le %10.3lg %10.3lf %-5Le%+e%+Le% Le",
      3456.4325, 3, 194.74543, 0.0, 58787.878958, 58787.878958, 58787.878958,
...
START_TEST(s21_sprintf_test_s) {
  ...
  int t1 =
      sprintf(str1, "%s %23s %-15s %10.s %15.2s %16.8s %7.5s %8s %15ls %15.3ls",
              "aboba", "helloworld", "ABOBA", "ABOBBAAA", "MASTICCU", "QWERTY",
              "CYNDISIGRID", "CHESTERHHH", s, s);
  int t2 = s21_sprintf(
      str2, "%s %23s %-15s %10.s %15.2s %16.8s %7.5s %8s %15ls %15.3ls",
      "aboba", "helloworld", "ABOBA", "ABOBBAAA", "MASTICCU", "QWERTY",
      "CYNDISIGRID", "CHESTERHHH", s, s);
  ck_assert_pstr_eq(str1, str2);
  ck_assert_int_eq(t1, t2);
...
START_TEST(s21_sprintf_test_p) {
  ...
  void *p4 = (void *)432520;
  int t1 = sprintf(str1, "%p %20p %-15p %10p %-15p %*p %10p %*p", p1, p2, p3,
                   p4, p2, 10, p3, p1, 10, p4);
  int t2 = s21_sprintf(str2, "%p %20p %-15p %10p %-15p %*p %10p %*p", p1, p2,
                       p3, p4, p2, 10, p3, p1, 10, p4);
...
START_TEST(s21_sprintf_test_x) {
  ...
  int t1 = sprintf(str1,
                   "%x %-25x %#10x %#-10x %#10.5x %-25.34x %#-10.2x "
                   "%*.*x%#*.10x %20.*x %#10.*x %*.5hx %10.25lx",
...
START_TEST(s21_sprintf_test1) {
  char str1[300] = {0};
  char str2[300] = {0};
  int r1 = sprintf(str1, "%d%s%c%i%f%u%%%s", 25, "aboba", 'a', 123, 4325.23434,
                   12345u, "hello my friend");
  int r2 = s21_sprintf(str2, "%d%s%c%i%f%u%%%s", 25, "aboba", 'a', 123,
...
START_TEST(s21_sprintf_test2) {
  ...
  void *p = (void *)0x3456;
  int n1, n2;
  int r1 = sprintf(string1, "%e %Lg %0G %x %X%n[%p]%%\n", 25.2587321,
                   (long double)4325.23434, 12345.00001, 8342, 1235, &n1, p);
  int r2 =
      s21_sprintf(string2, "%e %Lg %0G %x %X%n[%p]%%\n", 25.2587321,
                  (long double)4325.23434, 12345.00001, 8342, 1235, &n2, p);
  ck_assert_str_eq(string1, string2);
...
START_TEST(s21_sprintf_test_space) {
...
int main() {
  ...
  // sprintf
  TCase *tc_sprintf;
  tc_sprintf = tcase_create("sprintf func");
  suite_add_tcase(s1, tc_sprintf);
  tcase_add_test(tc_sprintf, s21_sprintf_test_d);
  tcase_add_test(tc_sprintf, s21_sprintf_test_i);
  tcase_add_test(tc_sprintf, s21_sprintf_test_s);
  tcase_add_test(tc_sprintf, s21_sprintf_test_c);
  tcase_add_test(tc_sprintf, s21_sprintf_test_u);
  tcase_add_test(tc_sprintf, s21_sprintf_test_p);
  tcase_add_test(tc_sprintf, s21_sprintf_test_o);
  tcase_add_test(tc_sprintf, s21_sprintf_test_x);
  tcase_add_test(tc_sprintf, s21_sprintf_test_percent);
  // tcase_add_test(tc_sprintf, s21_sprintf_test_cyndisig);
  ...
  tcase_add_test(tc1_1, s21_sprintf_test_space);

  // tcase_add_test(tc1_1, s21_sprintf_test_cyndisig);
